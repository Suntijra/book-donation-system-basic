...
module.exports = {
  ...
  findAdminUser: async (user, pwd) => {
    const [rows, fields] = await db.execute(`SELECT id,fname,lname,email,phoneno,lineID,level FROM user where username = ? and password = ? and level = 'admin'`, [user, pwd]);
    return [rows, fields]
  }
};